...
class ObjectStorageSample:
    ...
    def put_big_object(self, bucket_name, object_name, source_file_path, request_parameters=None):
        http_method = 'PUT'
        ...
        def generate_file_data(file_path):
            ...
            with open(file_path, 'rb') as f:
                while True:
                    data = f.read(1024*1024)  # 1MB 크기로 청크를 읽음
                    if not data:
                        break
                    uploaded += len(data)
                    yield data
                    print(f"Uploaded {uploaded} of {total_size} bytes ({(uploaded/total_size)*100:.2f}%)")
        ...
        request_url = self.endpoint + request_path
        r = requests.put(request_url, headers=headers, params=request_parameters, data=generate_file_data(source_file_path))
...
if __name__ == '__main__':
    ...
    sample.put_big_object('data-lake', 'bronze/eventsim_big.csv', './eventsim.csv')
    # sample.put_object('data-lake', 'bronze/eventsim.csv', './eventsim.csv')
